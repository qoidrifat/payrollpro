...
import { CalendarDaysIcon, PlusIcon, XMarkIcon, PaperAirplaneIcon } from '@heroicons/vue/24/outline'
...
const closeForm = () => { showForm.value = false; form.reset() }
</script>
    ...
    <component :is="Layout">
        <div class="max-w-5xl mx-auto space-y-8">
            ...
            <div class="glass-card overflow-hidden">
                <div class="h-1.5 bg-gradient-to-r from-sky-500 to-blue-600" />
                <div class="p-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <h1 class="text-xl lg:text-2xl font-extrabold text-gray-900 dark:text-white">Cuti & Izin</h1>
                                <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Ajukan dan pantau status cuti Anda</p>
                            </div>
                        </div>
            ...
            <!-- Form -->
            <div v-if="showForm" class="glass-card overflow-hidden">
                <div class="h-1 bg-gradient-to-r from-sky-500 to-cyan-500" />
                <div class="p-6">
                    <div class="flex items-center justify-between mb-5">
                        <h3 class="text-base font-bold text-gray-900 dark:text-white">Form Pengajuan Cuti</h3>
                        <button @click="closeForm" class="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                    ...
                    <form @submit.prevent="submitLeave" class="space-y-5">
                        <div>
                            ...
                            <p v-if="form.errors.leave_type" class="form-error">{{ form.errors.leave_type }}</p>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            <div>
                                <label class="form-label">Tanggal Mulai</label>
                                <input v-model="form.start_date" type="date" class="form-input" required />
                                <p v-if="form.errors.start_date" class="form-error">{{ form.errors.start_date }}</p>
                            </div>
                            <div>
                                <label class="form-label">Tanggal Selesai</label>
                                <input v-model="form.end_date" type="date" class="form-input" required :min="form.start_date" />
                                <p v-if="form.errors.end_date" class="form-error">{{ form.errors.end_date }}</p>
                            </div>
                        </div>
                        <div v-if="totalDays > 0" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-primary-50 dark:bg-primary-950/60 text-sm font-semibold text-primary-700 dark:text-primary-300">
                        ...
                        <div class="flex justify-end gap-3 pt-2">
                            <button @click="closeForm" type="button" class="btn-ghost text-sm">Batal</button>
            ...
            <div class="glass-card overflow-hidden">
                <div class="px-6 pt-6 pb-2">
                    <h3 class="text-base font-bold text-gray-900 dark:text-white">Riwayat Pengajuan</h3>
                </div>
                <div v-if="records.length" class="divide-y divide-gray-100 dark:divide-gray-800">
                    ...
                    >
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-sm font-semibold text-gray-900 dark:text-white">{{ leaveTypeLabel(leave.leave_type) }}</p>
                ...
                <p v-else class="text-sm text-gray-500 dark:text-gray-400 text-center py-10">Belum ada pengajuan cuti.</p>
            </div>
        </div>
    </component>
</template>
