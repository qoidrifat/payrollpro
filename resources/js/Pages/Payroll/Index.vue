...
import { PlusIcon, EyeIcon, TrashIcon, CurrencyDollarIcon } from '@heroicons/vue/24/outline'
...
const deletePayroll = () => {
    ...
    router.delete(route('payroll.destroy', deleteTarget.value.id), {
        onFinish: () => { deleting.value = false; showDeleteConfirm.value = false; deleteTarget.value = null },
    })
}
</script>
            ...
            >
                ...
                <template #cell-actions="{ row }">
                    <div class="flex items-center gap-2">
                        <Link :href="route('payroll.show', row.id)" class="btn-secondary text-xs py-1.5 px-3">
                            <EyeIcon class="w-4 h-4" />
                            Lihat
                        </Link>
                        <button v-if="row.status === 'draft'" @click="confirmDelete(row)" class="btn-danger text-xs py-1.5 px-3">
